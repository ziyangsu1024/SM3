...
append_m = "201900210030"   # 随机的附加消息
pad_str = ""
pad = []

def padding(msg):    #构造补充部分
    mlen = len(msg)
    msg.append(0x80)
    mlen += 1
    tail = mlen % 64
...
def generate_guess_hash(old_hash, secret_len, append_m):
    """
       1.生成一个secret，算出secret的sm3值
        2.根据sm3值分组后得到各个寄存器里的值
        3.在secret+padding之后附加一段消息，用上一步寄存器里的值作为IV去加密附加的那段消息，得到m1
        4.再用sm3去加密secret+padding+m'，得到m2
        5.若m1和m2相等，则攻击成功
    """
    vectors = []
    message = ""
    ...
    return store.sm3_hash(message, vectors)
